...
test("production configuration accepts an exact secure contract", () => {
  ...
  for (const name of [
    "DB_NAME", "OPENAI_MODEL", "OPENAI_TIMEOUT_MS", "OPENAI_MAX_RETRIES",
  ...
  assert.equal(compact.openaiModel, "gpt-4o-mini-2024-07-18");
...
test("emailed links cannot be built from a localhost origin in production", () => {
  ...
  const withOrigins = (origins) => {
    ...
    Object.assign(process.env, {
      ...
      OPENAI_API_KEY: "p", OPENAI_MODEL: "gpt-4o-mini-2024-07-18",
