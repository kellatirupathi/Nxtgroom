  ...
  return (
    <section className="w-full flex flex-col h-full" aria-labelledby="boa-title">
      <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
        ...
        <button type="button" onClick={openCreateModal} disabled={colleges.length === 0} className="bg-indigo-600 text-white px-4 py-2.5 rounded-md font-bold text-sm flex items-center gap-2 hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {!loading && colleges.length === 0 && <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-medium text-amber-700">Create a college before adding a BOA.</div>}
                ...
                <tr key={boa._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button type="button" aria-label={`Delete ${boa.name}`} title={`Delete ${boa.name}`} disabled={Boolean(deletingId)} onClick={() => handleDelete(boa)} className="rounded-md border border-rose-100 bg-rose-50 p-2 text-rose-700 hover:bg-rose-100 focus:outline-none focus:ring-2 focus:ring-rose-500 disabled:opacity-50"><Trash2 size={16} aria-hidden="true" /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white rounded-md shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={closeModal} disabled={submitting} className="flex-1 px-4 py-3 rounded-md font-bold text-sm text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors disabled:opacity-50">Cancel</button>
                <button type="submit" disabled={submitting} className="flex-1 px-4 py-3 rounded-md font-bold text-sm text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200 disabled:opacity-50">{submitting ? 'Saving…' : isEditMode ? 'Save Changes' : 'Create BOA'}</button>
              </div>
            </form>
          </div>
        </div>
